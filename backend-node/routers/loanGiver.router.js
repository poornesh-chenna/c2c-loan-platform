...
router.post("/accept-loan", authorizeUser, async (req, res) => {
  ...
  res
    .status(200)
    .send({ message: "You are now Successfully accepted the loan" });
});

router.patch("/modify-loan", authorizeUser, (req, res) => {
  Loan.updateOne(
    { _id: req.body.loanId },
    {
      $addToSet: {
    ...
    (err) => {
      if (err) {
        ...
        res
          .status(200)
          .send({ message: "successfully modified loan is updated" });
      }
    }
  );
});
